...
void dencrypt(unsigned char* image, const unsigned char* randIm1, unsigned char* randIm2,
	unsigned int width, unsigned int height);

void antiGraveScale(unsigned char* image, unsigned int width, unsigned int height);
const graveScl = 0x00010101;
...
int main(int argc, char* argv[])
{
	...
	loadImage(filenameOut1, &rand1, &width, &height);
	loadImage(filenameOut2, &rand2, &width, &height);
	...
	if (!(shrunkImage == NULL || rand1 == NULL || rand2 == NULL))
	{
		...
		////antiGraveScale(shrunkImage, width / 2, height);
		...
		dencrypt(image, rand1, rand2, width / 2, height);

		writeImage(filenameIn, image, width, height);
	}
	...
	unsigned int width, unsigned int height)
{
	...
	for (row = 0; row < height; row++)
	{
		for (col = 0; col < width; col++)
		{
			if (randIm2[row * width + col] == 255)
			{
				image[row * width + col] = randIm2[row * 2 * width + 2 * col] ^ randIm1[row * 2 * width + 2 * col];
			}
		}
	}
}
...
void antiGraveScale(unsigned char* image, unsigned int width, unsigned int height)
{
	...
	for (row = 0; row < height; row++)
	{
		for (col = 0; col < width; col++)
		{
			image[row * width + col] = (image[row * width + col] > 128) * graveScl;
		}
	}
}
